...
class ForexTradingEnv(gym.Env):
    ...
    def __init__(
        ...
        lot_size: float = 1.0,            # 1.0 lot = 100k units (for equity in $)
        reward_scale: float = 0.1,             # optional scaling of rewards
        ...
        time_penalty_pips: float = 0.01,     # NEW: cost per bar in a trade
    ):
        super().__init__()

        self._bar_times = self._extract_bar_times(df.index)
    ...
    def _extract_bar_times(self, index) -> list[datetime] | None:
        ...
        try:
            ...
            if isinstance(ts, pd.DatetimeIndex):
                ...
                return list(ts.to_pydatetime())
        except Exception:
            return list(index)
        return list(index)

    def _bar_time_iso(self, step: int) -> str | None:
        if self._bar_times is None or not (0 <= step < len(self._bar_times)):
            return None
        try:
            return pd.Timestamp(self._bar_times[step]).isoformat()
        except Exception:
            return str(self._bar_times[step])
    ...
    def _reset_state(self):
        ...
        # Accounting
        self.initial_equity_eur = 10000.0
    ...
    def _get_state_features(self):
        ...
        t_norm = float(self.time_in_trade) / 1 # 1000.0
    ...
    def _open_position(self, direction: int, sl_pips: float, tp_pips: float):
        ...
        self.last_trade_info = {
            ...
            "timestamp": self._bar_time_iso(self.current_step),
        }
    ...
    def _close_position(self, reason: str, exit_price: float):
        ...
        self.equity_eur += net_pips * self.usd_per_pip
        ...
        trade_info = {
            ...
            "equity_eur": float(self.equity_eur),
    ...
    def step(self, action: int):
        ...
        # 6) Log equity
        self.equity_curve.append(float(self.equity_eur))
    ...
    def render(self):
        print(
            f"Step={self.current_step} | Equity=${self.equity_eur:,.2f} | "
